...
- **lawmasolutions.com is live** over HTTPS, serving a styled "Coming Soon" page. Stack now includes: FastAPI app in a Docker container on Fly.io (`iad`, free tier, scales to zero), reachable at the apex + `www` via Squarespace DNS pointing at Fly's anycast IPs, Let's Encrypt cert auto-issued. Resend domain verified for sending; `RESEND_API_KEY` set as a Fly secret. Full walkthrough captured in `design/deployment.md`. Items 9 and 10 flipped to `[~]` (functionally complete; awaiting Tom's signoff).
- **Stack still missing**: persistent storage. Fly machines have ephemeral filesystems and `auto_stop_machines = "stop"` makes that more aggressive — anything written to the local disk during a request is gone on next redeploy. Item 4 (SQLAlchemy + Alembic + SQLite-on-Fly-volume) is the real fix. Pilot stopgap option: create a small Fly volume + mount in `fly.toml`, store item 21's `exclusions.json` / `edits.json` there. Either way, this work blocks item 21 the moment we need to persist any state across deploys.
